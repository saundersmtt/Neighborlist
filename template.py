...
sele1 = u.select_atoms('resname {}'.format(args.reference))
...
for i in range(num_atoms_sele1):
    sele1_residues[i] = sele1.atoms[i].resid
    sele1_names.append(sele1.atoms[i].resname)

sele2_residues = np.zeros(num_atoms_sele2,dtype=int)
sele2_names = []
for i in range(num_atoms_sele2):
    sele2_residues[i] = sele2.atoms[i].resid
    sele2_names.append(sele2.atoms[i].resname)
